...
#[inline]
fn advance_unchecked(s: &mut &[u8], n: usize) {
    unsafe {
        let ptr = s.as_ptr().add(n);
        let len = s.len() - n;
        *s = slice::from_raw_parts(ptr, len);
    }
}
...
impl FromRadix10Checked for u16 {
    #[inline]
    fn from_radix_10_checked(mut s: &[u8]) -> Result<Self, ()> {
        let mut is_err = 0;
        let res: u64 = match s.len() {
            1 => parse_1(&mut s, &mut is_err),
            2 => parse_2(&mut s, &mut is_err),
            3..=5 => parse_u16(&mut s, &mut is_err),
            _ => {
                strip_leading_zeros(&mut s, 5);
                if s.is_empty() || s.len() > 5 {
                    return Err(());
                }
                parse_u16(&mut s, &mut is_err)
            }
        };
        match is_err {
            0 => Ok(res as u16),
